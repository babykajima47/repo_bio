<?php
/**
 * Template: Minimal Light (slug: minimal) — mặc định.
 * Nhận sẵn từ renderBioPage(): $profile (array), $links (array rows DB), $themeColor (string), $csrf (string).
 * Chỉ render HTML/CSS — không query DB, không viết lại JS click/lead (dùng chung bioPageScript()).
 */

$avatarHtml = $profile['avatarPath']
    ? '<img src="' . e($profile['avatarPath']) . '" alt="' . $profile['name'] . '" class="h-24 w-24 rounded-full object-cover" style="border:3px solid white;box-shadow:0 0 0 1px #E5E7EB">'
    : '<div class="flex h-24 w-24 items-center justify-center rounded-full text-3xl font-bold text-white" style="background:' . e($themeColor) . '">' . $profile['initial'] . '</div>';
...
if (!empty($profile['hotlinePhone'])) {
    $quickButtons .= '<a href="tel:' . e(normalizePhoneVn($profile['hotlinePhone'])) . '" class="flex flex-1 items-center justify-center gap-2 rounded-lg px-4 py-2.5 text-sm font-semibold text-white transition hover:opacity-90" style="background:#16A34A">' . icon('phone', 'h-4 w-4', 'none') . '<span>Gọi Hotline</span></a>';
}
if (!empty($profile['zaloPhone'])) {
    $quickButtons .= '<a href="' . e(zaloUrl($profile['zaloPhone'])) . '" target="_blank" rel="noopener" class="flex flex-1 items-center justify-center gap-2 rounded-lg px-4 py-2.5 text-sm font-semibold text-white transition hover:opacity-90" style="background:#0068FF">' . icon('messageCircle', 'h-4 w-4', 'none') . '<span>Nhắn Zalo</span></a>';
}
...
?>
<div class="relative min-h-screen overflow-hidden" style="background:#F7F8FA">
  <div class="pointer-events-none absolute inset-0 overflow-hidden">
    <div class="absolute rounded-full" style="width:280px;height:280px;top:-90px;left:-90px;background:<?= e($themeColor) ?>;opacity:.14;filter:blur(46px)"></div>
    <div class="absolute rounded-full" style="width:220px;height:220px;top:140px;right:-70px;background:<?= e($themeColor) ?>;opacity:.10;filter:blur(46px)"></div>
    <div class="absolute rounded-full" style="width:200px;height:200px;bottom:80px;left:-60px;background:#38BDF8;opacity:.12;filter:blur(46px)"></div>
  </div>
  <div class="relative z-10 mx-auto flex w-full flex-col items-center px-5 pt-14" style="max-width:480px">
    <?= $avatarHtml ?>
    <h1 class="mt-4 flex items-center gap-1.5 text-lg font-bold" style="color:#111827"><?= $profile['name'] ?> <?= $verifiedBadge ?></h1>
    <p class="mt-1 text-sm font-medium" style="color:<?= e($themeColor) ?>"><?= $profile['jobTitle'] ?></p>
    <p class="mt-2 max-w-xs text-center text-sm leading-relaxed" style="color:#64748B"><?= $profile['bio'] ?></p>
